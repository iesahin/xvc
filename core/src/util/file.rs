...
mod tests {
    ...
    #[test]
    fn test_glob_includes() {
        let pmp = XvcPathMetadataProvider::new(&xvc_root).unwrap();
        let pipeline_rundir = XvcPath::new(&xvc_root, &xvc_root, "/tmp/xvc/pipeline").unwrap();
        let glob = "*.py";
        let path = XvcPath::new(&xvc_root, &xvc_root, "/tmp/xvc/pipeline/src/main.py").unwrap();
        assert!(glob_includes(&xvc_root, &pmp, &pipeline_rundir, glob, &path).unwrap());
    }
    #[test]
    fn test_dir_includes() {
        let xvc_root = XvcRoot::new("/tmp/xvc").unwrap();
        let pmm = XvcPathMetadataMap::new();
        let directory = XvcPath::new(&xvc_root, &xvc_root, "/tmp/xvc/pipeline").unwrap();
        let path = XvcPath::new(&xvc_root, &xvc_root, "/tmp/xvc/pipeline/src/main.py").unwrap();
        assert!(dir_includes(&pmm, &directory, &path).unwrap());
    }
    #[test]
    ...
    #[test]
    fn test_path_metadata_channel() {
        let (sender, receiver) = bounded(CHANNEL_BOUND);
        let start_dir = PathBuf::from("/tmp/xvc");
}
